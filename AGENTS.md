...
For V3 changes, clue-only and equation baselines share neutral candidates and prompts. LLM + Jev receives the full visible board and uses only LLM-proposed candidates, without code frontier selection or equations. No V3 mode may call the solver or risk scorer. Validate proposals without repairing, ranking, or replacing them; model advice is unverified. Keep V3 sessions independent of the original browser experiments.
